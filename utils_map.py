...
def get_map(MINOVERLAP, draw_plot, score_threhold=0.5, path = './map_out'):
    ...
    ground_truth_files_list.sort()#获得文件名称并排序
    ...
    for txt_file in ground_truth_files_list:
        ...
        temp_path   = os.path.join(DR_PATH, (file_id + ".txt"))#规范化
        if not os.path.exists(temp_path):
            error_msg = "Error. File not found: {}\n".format(temp_path)
            error(error_msg)
        lines_list      = file_lines_to_list(txt_file)#每一行
        bounding_boxes  = []
        is_difficult    = False
        ...
        for line in lines_list:
            ...
            if is_difficult:
                ...
                if class_name in gt_counter_per_class: #记录各类别个数
    ...
    dr_files_list = glob.glob(DR_PATH + '/*.txt')#检测结果
    dr_files_list.sort()
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            dr_data = json.load(open(dr_file)) #检测到的框
            ...
            for idx, detection in enumerate(dr_data):
                ...
                ground_truth_data   = json.load(open(gt_file))#真实框
                ovmax       = -1
                gt_match    = -1
                ...
                for obj in ground_truth_data:
                    if obj["class_name"] == class_name:
                        ...
                        box1    = np.array([bb[0], bb[1], bb[2], bb[3], bb[4]], np.float32)  #xc,yc,w,h,angle
                        box2    = np.array([bbgt[0], bbgt[1], bbgt[2], bbgt[3], bbgt[4]], np.float32)
                        ov      = iou_rotate_calculate(box1, box2)
                        if ov > ovmax:#遍历每一个真实框  找重叠度最大的那个
                            ovmax = ov
                            gt_match = obj
                ...
                if ovmax >= min_overlap:
                    if "difficult" not in gt_match:
                        if not bool(gt_match["used"]):#这个真实框是否被使用
                            tp[idx] = 1
                            gt_match["used"] = True
                            ...
                            bbgt_1    = [float(x) for x in gt_match["bbox"].split() ]
                            box1_1    = np.array([bb[0], bb[1], bb[2], bb[3], bb[4]], np.float32)  #xc,yc,w,h,angle
                            box2_2  = np.array([bbgt_1[0], bbgt_1[1], bbgt_1[2], bbgt_1[3], bbgt_1[4]], np.float32) 
                            
                            # f1.write(str(box1_1[4]))
                            # f1.write('  ')
                            # f1.write(str(box2_2[4]))
                            # f1.write('\n')
                            # f1.close()
                            ...
                            #规范化                            
                            angle_de = abs(box1_1[4]-box2_2[4])
                            if angle_de>60:
                                angle_de = abs(angle_de-90)
                                if(box1_1[4]<box2_2[4]):
                                    angle_de_per = angle_de/box2_2[4]
                                else:
                                    angle_de_per = angle_de/(box2_2[4]+90) 
                            else:
                                if box2_2[4]==0:
                                    angle_de_per = angle_de/90
                                else:
                                    angle_de_per = angle_de/box2_2[4]
                            # f = open(file_path,'a+')
                            # f.write(str(box1_1[4]))
                            # f.write('  ')
                            # f.write(str(box2_2[4]))
                            # f.write('   ')
                            # f.write(str(angle_de))
                            # f.write('\n')
                            # f.close
                            angle_des.append(angle_de)
                            angle_des_per.append(angle_de_per)
    ...
    #输出并记录角度
    angle_re = sum(angle_des)/len(angle_des)
    print(sum(angle_des),len(angle_des))
    angle_per_re = sum(angle_des_per)/len(angle_des_per)
    print(np.sum(angle_des_per),len(angle_des_per))
    print("角度偏差为：")
    print(angle_re,angle_per_re)

    sorted_arr = np.sort(angle_des_per)[::-1]
    np.savetxt('output.txt', sorted_arr, delimiter='\n')
    with open('filename.txt', 'w') as f:
        for i, item in enumerate(sorted_arr, 1):
            ...
            if i % 10 == 0:
                ...
                f.write(' ')
    # top_10_max_elements = sorted(enumerate(angle_des), key=lambda x: x[1], reverse=True)[:10]
    # # 输出前十个最大元素及其索引
    # for idx, value in top_10_max_elements:
    #     print("元素：", value, "索引：", idx)
    # print('   ')
    # top_10_max_elements_p = sorted(enumerate(angle_des_per), key=lambda x: x[1], reverse=True)[:10]
    # # 输出前十个最大元素及其索引
    # for idx, value in top_10_max_elements_p:
    #     print("元素：", value, "索引：", idx)  
    """
